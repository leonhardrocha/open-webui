...
from open_webui.utils.misc import pop_system_message, get_last_user_message_item, get_last_assistant_message_item, get_content_from_message, get_message_list
    ...
    async def pipe(
            ...
            if __task__ is not None:
                if __task__ == "title_generation":
                    await status_event.curry(description="Geração de título pelo Dify (placeholder).").emit()
                    yield {"type": "text", "content": f"Título Dify: {model_name}"}
                    return
                elif __task__ == "tags_generation":
                    await status_event.curry(description="Geração de tags pelo Dify (placeholder).").emit()
            ...
            chat_info_json = await self.openwebui.get_chat_info(event_emitter, body)
            
            chat_info = chat_info_json.get("chat_info")
            if not chat_info:
                error_msg = "Erro: Resultado da conversa não encontrado."
                self.logger.error(error_msg)
                await status_event.curry(description=error_msg).emit()
                yield {"type": "error", "content": error_msg}
                return
            ...
            if not messages:
                error_msg = "Erro: Mensagens da conversa não encontradas."
                self.logger.error(error_msg)
                await status_event.curry(description=error_msg).emit()
                yield {"type": "error", "content": error_msg}
                return
            ...
            if "upload_files" in body and body["upload_files"]:
                ...
                for file_obj in body["upload_files"]:
                    ...
                    if not file_path or not os.path.exists(file_path):
                        ...
                        self.logger.error(error_msg)
                        await status_event.curry(description=f"Erro: Arquivo carregado não encontrado no servidor: {file_obj.get('file',{}).get('filename', 'Arquivo desconhecido')}").emit()
                    ...
                    try:
                        if self.dify.is_doc_file(file_path):
                            ...
                            await status_event.curry(description=f"Pulando arquivo não suportado: {file_name}").emit()
                            continue
            ...
            try:
                ...
                async for chunk in self.dify.send_chat_message(
                    ...
                    if event_type in ["message", "agent_message"]: # Handle both message and agent_message as text output [2]
                        ...
                        await status_event.curry(description=f"Nó Dify Iniciado: {node_name}").emit()
                        ...
                        await status_event.curry(description=f"Nó Dify Finalizado: {node_name} (Status: {status})" + (f" Erro: {error_msg}" if error_msg else "")).emit()
                        ...
                        await status_event.curry(description=f"Iteração Iniciada: {node_name}").emit()
                        ...
                        await status_event.curry(description=f"Próxima Iteração: {node_name} (Índice: {index})").emit()
                        ...
                        await status_event.curry(description=f"Iteração Concluída: {node_name} (Status: {status})" + (f" Erro: {error_msg}" if error_msg else "")).emit()
                        ...
                        await status_event.curry(description=f"Ramificação Paralela Iniciada: {branch_id}").emit()
                        ...
                        await status_event.curry(description=f"Ramificação Paralela Finalizada: {branch_id} (Status: {status})" + (f" Erro: {error_msg}" if error_msg else "")).emit()
                        ...
                        node_name = node_data.get("title", "Loop")
                        index = node_data.get("index")
                        await status_event.curry(description=f"Próximo Loop: {node_name} (Índice: {index})").emit()
                        ...
                        status = node_data.get("status")
                        error_msg = node_data.get("error")
                        await status_event.curry(description=f"Loop Concluído: {node_name} (Status: {status})" + (f" Erro: {error_msg}" if error_msg else "")).emit()
                        yield {"type": "loop_finish", "content": node_data}

                    elif event_type == "node_retry": # Node retry event [2]
                        node_data = chunk.get("data", {})
                        node_name = node_data.get("title", "Node")
                        await status_event.curry(description=f"Tentando Novamente Nó: {node_name}").emit()
                        yield {"type": "node_retry", "content": node_data}

                    elif event_type == "text_replace": # Text replacement event [2]
                        await status_event.curry(description="Substituição de Texto...").emit()
                    ...
                    # handle_event already returns them in a suitable format, so we can yield them directly.
                        ...
                        await status_event.curry(description="Substituição de Arquivo...").emit()
...
class OpenWebUIHelper:
    ...
    def __init__(self, valves: Tools.Valves, debug: bool = False):
        ...
        self.data_cache_dir = "~/.cache/"  # Directory to persist state

    async def get_chat_info(self, event_emitter: EventEmitter, body: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        chat_info = self.process_conversation_state(chat_id, body)
        if chat_info.get('error_message'):
            await status_event.curry(description=chat_info.get('error_message')).emit()
    ...
    def process_conversation_state(self, chat_id: str, body: Dict[str, Any]) -> dict:
        """
        Process and manage Dify conversation state.
        ...
        Returns:
            dict: A dictionary containing:
                - conversation_id: str with conversation ID if exists, None otherwise. This can be filled by dify response.
                - message_id_map: dict mapping OpenWebUI message IDs to Dify message IDs
                - user_message_id: str with user message ID if exists, None otherwise
                - user_message_content: str with user message content if exists, None otherwise
                - assistant_message_id: str with assistant message ID if exists, None otherwise
                - assistant_message_content: str with assistant message content if exists, None otherwise
        """
        result = {
        ...
        self.logger.debug(f"Total other messages: {len(messages)}")

        last_user_message_content=""
        if last_user_message := get_last_user_message_item(messages):
            last_user_message_content = get_content_from_message(last_user_message)

        last_assistant_message_content=""
        if last_assistant_message := get_last_assistant_message_item(messages):
            last_assistant_message_content = get_content_from_message(last_assistant_message)

        # Initialize or retrieve conversation state
        if chat_id not in self.chat_message_mapping: 
            conversation_id = None
            message_id_map = {}           
            self.chat_message_mapping[chat_id] = {
                "conversation_id": None,  # Will be filled by Dify's first response
                "message_id_map": message_id_map,  # Map OWUI message_id to Dify message_id
            }
            self.file_list[chat_id] = {}  # Clear file list for new conversation
            self.logger.info(
                f"Nova conversa iniciada para chat_id: {chat_id}. Estado limpo."
            )
        else:

            # Get existing conversation ID and message map
            conversation_id = self.chat_message_mapping[chat_id].get("conversation_id")
            message_id_map = self.chat_message_mapping[chat_id].get("message_id_map", {})

            # Find the Dify message ID of the *previous user query* to set as parent for continuity
           
            if last_user_message_id and last_user_message_id in message_id_map:
                result['user_message_id'] = message_id_map[last_user_message_id]
                self.logger.debug(
                    f"Continuando conversa Dify com parent_message_id: {result['user_message_id']}"
                )
            else:
                self.logger.debug(
                    "Nenhuma mensagem anterior do usuário mapeada para Dify. Iniciando novo branch ou primeira mensagem da conversa Dify."
                )


            # Find the Dify message ID of the *previous AI response* to set as parent for continuity
            
            if last_assistant_message_id and last_assistant_message_id in message_id_map:
                result['assistant_message_id'] = message_id_map[last_assistant_message_id]
                self.logger.debug(
                    f"Continuando conversa Dify com parent_message_id: {result['assistant_message_id']}"
                )
            else:
                self.logger.debug(
        ...
        result['message_id_map'] = message_id_map
        result['conversation_id'] = conversation_id
        result['user_message_content'] = last_user_message_content
        result['assistant_message_content'] = last_assistant_message_content
...
class DifyHelper:
    ...
    class EventHandlerInterface(ABC):
        @abstractmethod
        def handle_event(
            self, event_content: dict, event_emitter: EventEmitter, chat_id: str, message_id: str
        ) -> Optional[Dict[str, Any]]:
            pass

    class BaseEventHandler(EventHandlerInterface):
        def __init__(self):
            self.logger = logging.getLogger()
        @abstractmethod
        def handle_event(
    ...
    class MessageFileEventHandler(BaseEventHandler):
        def handle_event(
            self, event_content: dict, event_emitter: EventEmitter, chat_id: str, message_id: str
        ) -> Optional[Dict[str, Any]]:
    ...
    class MessageEndEventHandler(BaseEventHandler):
        def handle_event(
            self, event_content: dict, event_emitter: EventEmitter, chat_id: str, message_id: str
        ) -> Optional[Dict[str, Any]]:
    ...
    class WorkflowStartEventHandler(BaseEventHandler):
        def handle_event(
            self, event_content: dict, event_emitter: EventEmitter, chat_id: str, message_id: str
        ) -> Optional[Dict[str, Any]]:
    ...
    class NodeStartEventHandler(BaseEventHandler):
        def handle_event(
            self, event_content: dict, event_emitter: EventEmitter, chat_id: str, message_id: str
        ) -> Optional[Dict[str, Any]]:
    ...
    class NodeFinishEventHandler(BaseEventHandler):
        def handle_event(
            self, event_content: dict, event_emitter: EventEmitter, chat_id: str, message_id: str
        ) -> Optional[Dict[str, Any]]:
    ...
    class WorkflowFinishEventHandler(BaseEventHandler):
        def handle_event(
            self, event_content: dict, event_emitter: EventEmitter, chat_id: str, message_id: str
        ) -> Optional[Dict[str, Any]]:
    ...
    class LoopStartEventHandler(BaseEventHandler):
        def handle_event(
            self, event_content: dict, event_emitter: EventEmitter, chat_id: str, message_id: str
        ) -> Optional[Dict[str, Any]]:
    ...
    class LoopFinishEventHandler(BaseEventHandler):
        def handle_event(
            self, event_content: dict, event_emitter: EventEmitter, chat_id: str, message_id: str
        ) -> Optional[Dict[str, Any]]:
    ...
    class ParallelBranchStartEventHandler(BaseEventHandler):
        def handle_event(
            self, event_content: dict, event_emitter: EventEmitter, chat_id: str, message_id: str
        ) -> Optional[Dict[str, Any]]:
            event_emitter.progress_update(f"Recebendo início da ramificação paralela do Dify...")
            self.logger.debug(f"Recebendo início da ramificação paralela do Dify: {event_content}")
            return event_content

    class ParallelBranchFinishEventHandler(BaseEventHandler):
        def handle_event(
            self, event_content: dict, event_emitter: EventEmitter, chat_id: str, message_id: str
        ) -> Optional[Dict[str, Any]]:
            event_emitter.progress_update(f"Recebendo fim da ramificação paralela do Dify...")
            self.logger.debug(f"Recebendo fim da ramificação paralela do Dify: {event_content}")
            return event_content    

    class IterationStartedEventHandler(BaseEventHandler):
        def handle_event(
            self, event_content: dict, event_emitter: EventEmitter, chat_id: str, message_id: str
        ) -> Optional[Dict[str, Any]]:
            event_emitter.progress_update(f"Recebendo início da iteração do Dify...")
            self.logger.debug(f"Recebendo início da iteração do Dify: {event_content}")
            return event_content

    class IterationNextEventHandler(BaseEventHandler):
        def handle_event(
            self, event_content: dict, event_emitter: EventEmitter, chat_id: str, message_id: str
        ) -> Optional[Dict[str, Any]]:
            event_emitter.progress_update(f"Recebendo próxima iteração do Dify...")
            self.logger.debug(f"Recebendo próxima iteração do Dify: {event_content}")
            return event_content

    class IterationCompletedEventHandler(BaseEventHandler):
        def handle_event(
            self, event_content: dict, event_emitter: EventEmitter, chat_id: str, message_id: str
        ) -> Optional[Dict[str, Any]]:
            event_emitter.progress_update(f"Recebendo fim da iteração do Dify...")
            self.logger.debug(f"Recebendo fim da iteração do Dify: {event_content}")
            return event_content

    class NodeRetryEventHandler(BaseEventHandler):
        def handle_event(
            self, event_content: dict, event_emitter: EventEmitter, chat_id: str, message_id: str
        ) -> Optional[Dict[str, Any]]:
    ...
    class TextReplaceEventHandler(BaseEventHandler):
        def handle_event(
            self, event_content: dict, event_emitter: EventEmitter, chat_id: str, message_id: str
        ) -> Optional[Dict[str, Any]]:
    ...
    def __init__(self, valves: Tools.Valves, debug: bool = False):
        ...
        self.event_handlers = {
            "message": self.MessageEventHandler(),
            "message_file": self.MessageFileEventHandler(),
            "message_end": self.MessageEndEventHandler(),
            "workflow_started": self.WorkflowStartEventHandler(),
            "workflow_finished": self.WorkflowFinishEventHandler(),
            "node_started": self.NodeStartEventHandler(),
            "node_finished": self.NodeFinishEventHandler(),            
            "loop_started": self.LoopStartEventHandler(),
            "loop_finished": self.LoopFinishEventHandler(),
            "parallel_branch_started": self.ParallelBranchStartEventHandler(),
            "parallel_branch_finished": self.ParallelBranchFinishEventHandler(),
            "iteration_started": self.IterationStartedEventHandler(),
            "iteration_next": self.IterationNextEventHandler(),
            "iteration_completed": self.IterationCompletedEventHandler(),
            "node_retry": self.NodeRetryEventHandler(),
            "text_replaced": self.TextReplaceEventHandler(),
            "error": self.ErrorEventHandler(),
        }
    ...
    def handle_event(
        ...
        return handler.handle_event(event_content, event_emitter, chat_id, message_id)
